...
// ===== Log =====
function renderLog(log, query = logSearchInput ? logSearchInput.value.trim().toLowerCase() : '') {
  logListEl.innerHTML = '';
...
if (copyAllLogBtn) copyAllLogBtn.addEventListener('click', async () => {
  const res = await sendToExtension('storageGet', { keys: [STORAGE_LOG_KEY] }).catch(() => null);
  const log = (res && res.data && res.data[STORAGE_LOG_KEY]) || [];
...
if (exportRulesBtn) exportRulesBtn.addEventListener('click', async () => {
  const res = await sendToExtension('storageGet', { keys: [STORAGE_RULES_KEY] });
  const rules = (res.success && res.data && res.data[STORAGE_RULES_KEY]) || [];
...
if (importRulesInput) importRulesInput.addEventListener('change', async () => {
